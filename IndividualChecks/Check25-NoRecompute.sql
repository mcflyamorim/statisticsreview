...
< ---------------- Description ----------------- >
NoRecompute disable the automatic statistics update option, AUTO_STATISTICS_UPDATE, for statistics_name. 
If this option is specified, the query optimizer will complete any in-progress statistics updates 
for statistics_name and disable future updates.

Using this option can produce suboptimal query plans. We recommend using this option sparingly, and then 
only by a qualified system administrator.

< -------------- What to look for and recommendations -------------- >
- Make sure there is a good and documented reason to use NoRecompute. It may be ok for LOB columns as they
usually take a lot of time to update/create.

- To re-enable statistics updates, remove the statistics with DROP STATISTICS and then run CREATE STATISTICS 
without the NORECOMPUTE option.
*/
