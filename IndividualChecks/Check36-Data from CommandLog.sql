/*
Check 36 - Check if Commandlog exists on master and return updatestats command and duration.
...
< -------------- What to look for and comments -------------- >
- Look for statistics taking a lot of time to update and make sure it is really being used. 
You may find, "bpk", "test", "backup" tables

- Check columns 
number_of_reads_on_index_table_since_last_restart, 
number_of_modifications_on_index_table_since_last_restart, 
number_of_range_scans_since_last_restart_rebuild, 
number_of_page_latch_since_last_restart_rebuild and 
number_of_page_i_o_latch_since_last_restart_rebuild
as they can give you more info about table usage. If table is not really used, you may want to 
remove those statistics or remove them from maintenance plan.

- A more drastic approach would just to go ahead and drop statistics taking more time to update, 
or maybe, drop them all and rely on auto-create stats to re-create really needed stats.
This may sound like a radical idea at first, but think about it. What do you have to gain or lose? 
Dropping all auto stats will place some temporary stress on the system. 
As queries come in, the query optimizer will begin recreating those statistics that we just dropped. 
Every query that a query requires a statistics to be created, it will wait. 
Soon, typically in a matter of minutes for highly utilized systems, most of the missing 
statistics will be already back in place and the temporary stress will be over. 
But now, only the ones that are really needed by the current workload will be 
re-created and all the redundant ones just came off the expensive maintenance tab.
If you are worried about the impact of the initial statistics creation, you can perform the cleanup
at off-peak hours and ‘warm-up’ the database by capturing and replaying a trace 
of the most common read only queries. 
This will create many of the required statistics objects without impacting your ‘real’ workload.
*/
